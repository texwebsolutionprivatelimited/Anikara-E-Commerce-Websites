...
export default function SkeletonLoader({ count = 8 }) {
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 sm:gap-x-4 gap-y-6 sm:gap-y-8 w-full">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="flex flex-col w-full font-sans">
          {/* Image skeleton */}
          <div className="relative aspect-[3/4] w-full rounded-sm skeleton-bg animate-skeleton bg-neutral-200" />
