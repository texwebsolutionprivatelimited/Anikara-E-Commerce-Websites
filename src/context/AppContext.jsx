  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Keep transparency if file is PNG
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          reject(new Error("Failed to parse response from ImageKit"));
        }
      } else {
        reject(new Error(`ImageKit Upload failed: ${xhr.statusText} - ${xhr.responseText}`));
      }
    };

    xhr.onerror = () => reject(new Error("Network error during upload to ImageKit"));
    xhr.send(formData);
  });
};
...
export const deleteFromImageKit = async (fileUrl) => {
  try {
    ...
    if (!response.ok) {
      const errText = await response.text();
      throw new Error(`Failed to delete image: ${errText}`);
    }
    return true;
  } catch (err) {
...
export const getResponsiveImageUrl = (url, width, height) => {
  if (!url || !url.includes("imagekit.io")) return url;
  const params = [];
  if (width) params.push(`w-${width}`);
  if (height) params.push(`h-${height}`);
  params.push("q-80"); // Optimize quality
